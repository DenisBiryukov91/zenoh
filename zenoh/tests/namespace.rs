...
async fn zenoh_namespace_pub_sub_inner(
    ...
    println!("zenoh_namespace_pub_sub: {ke1} -> {ke2}");
    let publisher = session1.declare_publisher(ke1).await.unwrap();
    let listener = publisher.matching_listener().await.unwrap();
    let subscriber = session2.declare_subscriber(ke2).await.unwrap();
...
async fn zenoh_namespace_queryable_get_inner(
    ...
    println!("zenoh_namespace_queryable_get: {ke1} -> {ke2}");

    let querier = session2.declare_querier(ke2).await.unwrap();
    let listener = querier.matching_listener().await.unwrap();
    let queryable = session1.declare_queryable(ke1).await.unwrap();
